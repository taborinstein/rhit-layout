...
use smash::{
    app::{
        lua_bind::{CameraModule::get_player_no, *},
        BattleObjectModuleAccessor,
    },
    lua2cpp::L2CFighterAnimcmdEffectCommon_effect_AuraHitF,
};
use std::io::Write;
...
struct Store {
    ...
    maps: [Option<ControllerMapping>; 8]
}
impl Store {
    pub fn new() -> Self {
        Store {
            tcp: None,
            inputs: [PlayerInput::new(); 8],
            maps: [None; 8],
        }
    }
    pub fn open(&mut self) {
        self.tcp = Some(TcpStream::connect("127.0.1:7878").unwrap());
    ...
    pub fn update_controller(
        ...
        self.maps[n] = Some(mappings.clone());//Some(unsafe { *mappings }.clone());
            ...
            for i in 0..8 {
                if self.inputs[i].is_modified {
                    match tcp_w.write_all(&[
                        ...
                        self.inputs[i].bb3,
                        self.inputs[i].style,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_x } else { self.maps[i].unwrap().pro_x } as u8,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_a } else { self.maps[i].unwrap().pro_a } as u8,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_b } else { self.maps[i].unwrap().pro_b } as u8,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_y } else { self.maps[i].unwrap().pro_y } as u8,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_l } else { self.maps[i].unwrap().pro_l } as u8,
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_r } else { self.maps[i].unwrap().pro_r } as u8,
                        if self.inputs[i].style == 1 {                   13 as u8 } else { self.maps[i].unwrap().pro_zl as u8 },
                        if self.inputs[i].style == 1 { self.maps[i].unwrap().gc_z } else { self.maps[i].unwrap().pro_zr } as u8,
                        10,
                    ]) {
                        Ok(_) => {}
                        Err(_) => {
...
pub unsafe fn handle_final_input_mapping(
    ...
    let mappings = unsafe { *(_mappings.wrapping_add(player_idx as usize)) };
    original!()(_mappings, player_idx, out, controller_struct, arg);
    
    let b_bit_0: u8 = 0
        | (controller_struct.controller.current_buttons.dpad_up() as u8) << 0
    ...
    let b_bit_1: u8 = 0
        | (controller_struct.controller.current_buttons.l() as u8) << 0
    ...
    let b_bit_2: u8 = 0
        | (controller_struct.controller.current_buttons.stick_l() as u8) << 0
    ...
    let b_bit_3: u8 = 0
        | (controller_struct.controller.current_buttons.r_up() as u8) << 0
        ...
        | (controller_struct
            .controller
            .current_buttons
            .real_digital_l() as u8)
            << 4
        | (controller_struct
            .controller
            .current_buttons
            .real_digital_r() as u8)
            << 5;
